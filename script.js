...
fetch('http://127.0.0.1:5500/data.json')
    ...
    .then(json => {
        ...
        const html = data.thongTinThem.map(x => `<li class='nhomdinhtt'>${x.label}: ${x.value}</li>`).join(' ');
        thongTinThemDOM.innerHTML = html;

        const htmlImage = data.images.map(x => `<img src="${x}" , alt="" width="100" class="nhieuanhctsp">`).join(' ');
        imageContainerDOM.innerHTML = htmlImage;

        const cuoiCung = data.giaTien.map((x, index) => `
                <ul id="giaTienHai" class="cuoicung${index + 1}">
                    <li>${x.label} </li>
                    <li>${formatter.format(x.value)} </li>
                </ul>
        `).join(' ');
